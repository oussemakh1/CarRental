...
public function insert_reparation($data){
  ...
  //Check if car already in reparation
    $checkSql = "SELECT n_serie FROM reparation WHERE n_serie = ? AND date_sortie_garage < CURRENT_DATE()";
    $CheckCar = $this->db->select($checkSql, [$this->n_serie]);
    if(!$CheckCar) {
        //Query
        ...
        return error_message('Cette vehicule deja en reparation!');
    }
