...
public function insert_fournisseur($data){
  ...
  $CheckQuery = "SELECT societe,civilite FROM fournisseur WHERE  societe = $this->societe OR civilite = $this->civilite";
  $FetchFournisseur = $this->db->query($CheckQuery);
  if ($FetchFournisseur > 0 ) {
    return header("Location:?error_message=Ce fournisseur deja exist!");
  } else {
    ...
    //Error handling
    if($new_fournisseur > 0){
      return header('Location:fournisseurs.php?insert_success');
    }else{
      return header('Location:fournisseurs.php?insert_error');
    }
  }
...
public function update_fournisseur($id,$data){
  ...
  if ($fournisseur_update) {
    return header("Location:fournisseurs.php?update_success");
  }else{
    return header('Location:fournisseurs.php?update_error');
  }
...
public function delete_fournisseur($id){
  ...
  if($fournisseur_delete){
    return header("Location:fournisseurs.php?delete_success");
  }else{
...
public function fetch_Fournisseur($id){
    ...
    if($fetch_fournisseur){
      ...
      return header("Location:?error_message=Cette fournissuer n'exist pas!");
    }
}
...
public function fetch_allFournisseur(){
  ...
  if($all_Fournisseur){
    ...
    return header("Location:?error_message=il n'y a pas des fournisseurs pour le moment!");
  }
}
